...
class PricehomeComposer extends GrailsComposer {
    ...
    def afterCompose = { window ->
		...
		$('#searchMonyPoopuy').on('click',{
				...
				if (patient) {
					String s = patient.transfer.type[0]
					...
					$('#moneyYa').val(patient.prescription.total[0])
		...
		def results = Account.findAllByFname(session.user.fname)

		if(results){
				//$("grid > rows").detach()
				for(c in results.price[0])
					$("grid > rows").append{
								row{
									label(value:c.date)
									label(value:results[0].fname+" "+results[0].lname)
									label(value:c.patient.firstname+" "+c.patient.lastname)
									label(value:c.sumprice+" บาท")
									label(value:c.patient.tel)
								}
					}
		}
